...
| **补丁 6** | `patch_string_jump` | FastbootCmds.c | NOP 掉跳转到 `"is not allowed in Lock State"` 错误路径的条件分支（flash、erase、slot-change、snapshot-cancel 命令处理器）。 | 假锁状态下 fastboot 拒绝 flash/erase 命令。仅影响需要在假锁状态下重刷的救砖场景。 |
